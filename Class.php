...
class Display {
	...
	static function Ban($title, $versi, $server = 0){
		...
		if($api){
			...
			print bg_c1.str_pad($api->city.', '.$api->regionName.', '.$api->country, 44, " ", STR_PAD_BOTH).n;
		}
...
class Functions {
	...
	static function Tmr($tmr){date_default_timezone_set("UTC");$sym = [' 次のクレーム ',' 請求する秒数 ',' 次のクレーム ',' 請求する秒数 ',];$timr = time()+$tmr;$a = 0;while(true){$a +=1;$res=$timr-time();if($res < 1) {break;}print $sym[$a % 4].p.date('H',$res).":".p.date('i',$res).":".p.date('s',$res)."\r";usleep(100000);}print "\r           \r";}
